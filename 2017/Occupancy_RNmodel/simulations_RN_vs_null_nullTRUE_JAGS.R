...
# Comparison of Royle-Nichols model vs psi(.)p(.) model with JAGS
...
write.csv(result, "simulation_RN_vs_null_nullTRUE.csv")
dim(result)
head(result)
...
mean(result[, 'psiRN']) - psiTrue    # 0.123
sqrt(mean((result[, 'psi0'] - psiTrue)^2))   # 0.0418
sqrt(mean((result[, 'psiRN'] - psiTrue)^2))  # 0.1335
...
mean(err0)   # 0.0028
mean(errRN)  # 0.1239
sqrt(mean(err0^2))   # 0.0307
sqrt(mean(errRN^2))  # 0.1301

# LOOaic and WAIC
# How many 'bad's?
colMeans(result[, c("loo0bad", "waic0bad", "looRNbad", "waicRNbad")] > 0)
  # No bads for model0; RN model, looaic ok, waic 100% bad.
# How do they compare
